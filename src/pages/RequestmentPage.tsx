...
const RequestmentPage = () => {
  ...
  const reqList: Req[] = [
    {
      title: '고객님,\n당신의 이름을 알려주시겠어요?',
      placeholder: '예시) 김현호 (최대 8자)',
      maxLength: 8,
    },
    {
      title: '편지를 받을 분의\n이름을 알려주시겠어요?',
      placeholder: '예시) 박찬규 (최대 8자)',
      maxLength: 8,
    },
    {
      title: '편지가 향할 곳의\n주소가 어떻게 되나요?',
      placeholder: '예시) hyunhomon (@gmail.com 생략)',
      maxLength: 64,
    },
    {
      title: '편지를 작성하는\n목적은 무엇인가요?',
      placeholder: '예시) 부모님께 드릴 생신편지',
      maxLength: 100,
    },
    {
      title: '아래의 정보가\n정확한지 확인해주세요.',
    ...
    <KeyboardAvoidingView
      ...
      {index < 5 ? (
        <View>
          ...
          />
          <Text style={[Texts.Body20Bold, Style.Title]}>
            {reqList[index].title}
          </Text>
          {index < 4 ? (
            <TextInput
