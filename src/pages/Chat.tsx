...
const Chat = () => {
  ...
  let Messages: message[] = [
    ...
    {who: 'vi', text: 'adfaf', index: 2},
    {who: 'vi', text: 'adfaf', index: 3},
    {who: 'vi', text: 'adfaf', index: 4},
    {who: 'vi', text: 'adfaf', index: 5},
  ];

  // useEffect(() => {}, [violetMessages, myMessages]);
  ...
  return (
    <KeyboardAvoidingView
      ...
      <Screen>
        ...
        <ChatContainer>
          {Messages.map((items: message) => (
            <MyUnit message={items.text} />
          ))}
          <VioletUnit message={'짧은 문장'} />
          <MyUnit message={'짧은 문장'} />
          <VioletUnit
            message={
              '기이이이이이ㅣ이이ㅣ이이ㅣ이이잉이이이ㅣ이이이이이이이이인문장'
            }
          />
          <MyUnit
            message={
              '기이이이이이이이ㅣ이이ㅣ이이이ㅣ이이이이ㅣ이이이이이이ㅣ인문장'
            }
          />
        </ChatContainer>
        <InputBox placeholder={'메세지를 입력하세요 ...'} />
      </Screen>
    </KeyboardAvoidingView>
  );
};
